...
def split_data(X, y, train_size=0.7, val_size=0.15, test_size=0.15, random_state=42):
    ...
    # split remaining 30 in half, 15/15
...
train = X_train[categorical_features].mode()
train = X_train[numerical_features].median()
train
...
def impute_data(X_train, X_val, X_test, features, strategy="median"):
    ...
    if strategy == "median":
        ...
        impute = X_train[features].median().to_dict()
    elif strategy == "mode":
        #TODO a(ii) calculate mean
        impute = X_train[features].mode().iloc[0].to_dict()
    else:
        raise ValueError("strategy must be 'median' or 'mode'")

    #TODO a(ii) use fillna
    # print(train)
    X_train.loc[:, features] = X_train[features].fillna(impute)
    X_val.loc[:,   features] = X_val[features].fillna(impute)
    X_test.loc[:,  features] = X_test[features].fillna(impute)

    return X_train, X_val, X_test

# %%
# df = X_train.copy()
# impute_median = X_train[numerical_features].median().to_dict()
# impute_mode = X_train[categorical_features].mode().iloc[0].to_dict()

# # X_train.loc[:, numerical_features] = X_train[numerical_features].fillna(impute_median)
# # X_val.loc[:,   numerical_features] = X_val[numerical_features].fillna(impute_median.iloc[0])
# # X_test.loc[:,  numerical_features] = X_test[numerical_features].fillna(impute_median.iloc[0])

# df.loc[:, categorical_features] = df[categorical_features].fillna(impute_mode)
# # X_val.loc[:,   categorical_features] = X_val[categorical_features].fillna(impute_mode.iloc[0])
# # X_test.loc[:,  categorical_features] = X_test[categorical_features].fillna(impute_mode.iloc[0])
# df
...
def ridge_precompute(X_train_b, y_train):
    ...
    # print("HI: ", X_train_b.shape, y_train.shape, XtX.shape, Xty.shape, I.shape)
...
def ridge_solve_predict(XtX, Xty, I, best_lambda, X_test_b):
    ...
    """
    w_ridge = np.linalg.inv(XtX + (best_lambda * I)) @ Xty

    y_pred_ridge = X_test_b @ w_ridge
    # print("ridge: ", X_test_b.shape, w_ridge.shape, y_pred_ridge.shape)
...
def compute_gradient(X, y, w):
    ...
    """
    # Derive the loss function w.r.t w
    # Gradient = X^T * 2(Xw-y) / N
    N, d = X.shape

    # print("comp gradient:", X.shape, y.shape, w.shape, X.T.shape)
    # (1022, 242) (1022,) (242,)
...
def check_early_stop(w, w_prev, tol):
    ...
    """
    return np.abs(np.sum(w - w_prev)) < tol
...
def ista_lasso(X, y, alpha, n_iterations, lambda_val, tol=1e-8):
    ...
    for _ in range(n_iterations):
        ...
        # print("ista lasso", w.shape, grad.shape)
        w -= alpha * grad
...
y_train_log =  pd.Series(np.log1p(1+ y_train))
...
def smear_back_transform(y_train_log, train_pred_log, X_test_b, w):
    ...
    # smear = 1/n sum exp(e)
    smear = np.mean(np.exp(y_train_log - train_pred_log))
    # y_pred = (e^(Xw) - 1) * s
    y_pred_back = np.expm1(X_test_b @ w) * smear
...
y_train_log = np.log1p(1 + y_train)
y_val_log   = np.log1p(1 + y_val)
y_test_log  = np.log1p(1 + y_test)
